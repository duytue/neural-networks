...
# with xavier: initial acc = 30.682%
# without xavier: acc = 27.064%
x_axis = list(np.arange(0,50,1))
for xavier in [True, False]:
    model = create_model(xavier=xavier)
    if torch.cuda.is_available():
        model = model.cuda()
    ...
    if xavier == True:
        plt.plot(x_axis, acc_list, 'r', label='With Xavier')
    else:
        plt.plot(x_axis, acc_list, 'b', lavel='Without Xavier')

plt.legend()
plt.savefig('xavier.png')
